    ...
    */
    p5c.setup = function () {
        ...
        */
        ...
        //Menù first page buttons
        audioinput = p5c.createFileInput(handleFile);
        audioinput.position(p5c.width/2 - 100,p5c.height/2) ;
        audioinput.style('font-size', '15px', 'color', '#ffffff');

        let col = p5c.color(255,255,255);
        button = p5c.createButton('PLAY');
        button.style('font-size', '30px');
        button.style('border-radius', '30px');
        button.style('background-color', col);
        button.position(p5c.width/2 - 50,p5c.height/4*3);
    ...
    */
    p5c.draw = function () {
        ...
        if (started) {
            ...
            //we're in the menu
            p5c.fill(255, 255, 100)
            p5c.rectMode(p5c.CENTER)
            p5c.rect(p5c.width / 2, p5c.height / 2, p5c.width, p5c.height);
        }
    }
    function handleFile(file) {
        if (file.type == "audio") {
            audioin = p5c.loadSound(file);
        }

    }
    function playGame(){
        if (audioin.isLoaded() == true) {
            started == true;
            audioinput.remove();
            button.remove();
            p5c.userStartAudio();
